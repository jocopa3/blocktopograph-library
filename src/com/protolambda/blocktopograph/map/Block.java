...
public enum Block implements NamedBitmapProviderHandle, NamedBitmapProvider {
   ...
   */
    ...
    I_445_0_SHULKER_SHELL("shulker_shell", null, 445, 0, "items/shulker_shell.png"),
    ...
    //Use BufferedImage instead of Bitmap
    ...
    private static final HashMap<String, Block> byDataName = new HashMap<>();
    private static final HashMap<Integer, HashMap<Integer, Block>> blockMap;
    static {
        blockMap = new HashMap<>();
        HashMap<Integer, Block> subMap;
    ...
    public static void loadBitmaps() throws IOException {
        for(Block b : Block.values()){
            if(b.bitmap == null && b.texPath != null){
                try {
                    b.bitmap = ImageIO.read(new File(b.texPath));
                    //b.bitmap = BufferedImage.createScaledBitmap(BitmapFactory.decodeStream(assetManager.open(b.texPath)), 32, 32, false);
    ...
    public static Block getBlock(int id, int meta){
        if(id < 0) return null;
        HashMap<Integer, Block> subMap = blockMap.get(id);
